...
object ParquetApp {
  ...
  def main(args: Array[String]) {
    ...
    //分割线
    val parquetrdd = spark.read.format("parquet").load("/opt/cdh/spark/examples/src/main/resources/users.parquet")

    parquetrdd.show()

    parquetrdd.select("name","favorite_color").show()
    //转换为json  进行保存
    parquetrdd.write.format("json").save("/opt/cdh/spark/examples/src/main/resources/users_parquet.json")

    parquetrdd.write.saveAsTable("users_parquet")
